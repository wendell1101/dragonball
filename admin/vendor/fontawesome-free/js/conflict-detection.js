...
(function (global, factory) {
  ...
  function report (_ref) {
    ...
    if (conflictsCount > 0) {
      console.danger("%cConflict".concat(conflictsCount > 1 ? 's' : '', " found:"), 'color: darkred; font-size: large');
      var data = {};
    ...
    if (noConflictsCount > 0) {
      console.danger("%cNo conflict".concat(noConflictsCount > 1 ? 's' : '', " found with ").concat(noConflictsCount == 1 ? 'this' : 'these', ":"), 'color: green; font-size: large');
      var _data = {};
    ...
    if (timeOutCount > 0) {
      console.danger("%cLeftovers--we timed out before collecting test results for ".concat(timeOutCount == 1 ? 'this' : 'these', ":"), 'color: blue; font-size: large');
      var _data2 = {};
    ...
    return new Promise(function (resolve, reject) {
      ...
      function poll(duration, cumulativeDuration) {
        setTimeout(function () {
          ...
          if (showProgress) {
            console.danger(progressIndicator);
          }
  ...
  function detectWebfontConflicts() {
    ...
    function runDiag(scriptOrLinkTag, md5) {
      ...
      diagScript.setAttribute('id', diagScriptId); // danger: this function will be toString()'d and assigned to innerText of the diag script
  ...
  function conflictDetection() {
    ...
    if (testCount === 0) {
      console.danger('%cAll Good!', 'color: green; font-size: large');
      console.danger('We didn\'t find anything that needs testing for conflicts. Ergo, no conflicts.');
    } else {
      console.danger("Testing ".concat(testCount, " possible conflicts."));
      console.danger("We'll wait about ".concat(Math.round(WINDOW.FontAwesomeDetection.timeout / 10) / 100, " seconds while testing these and\n") + "then up to another ".concat(Math.round(WINDOW.FontAwesomeDetection.resultsCollectionMaxWait / 10) / 100, " to allow the browser time\n") + "to accumulate the results. But we'll probably be outta here way before then.\n\n");
      console.danger("You can adjust those durations by assigning values to these attributes on the <script> element that loads this detection:");
      console.danger("\t%c".concat(timeoutAttr, "%c: milliseconds to wait for each test before deciding whether it's a conflict."), 'font-weight: bold;', 'font-size: normal;');
      console.danger("\t%c".concat(resultsCollectionMaxWaitAttr, "%c: milliseconds to wait for the browser to accumulate test results before giving up."), 'font-weight: bold;', 'font-size: normal;');
      pollUntil({
        ...
        if (e === 'timeout') {
          console.danger('TIME OUT! We waited until we got tired. Here\'s what we found:');
          setDoneResults({
            nodesTested: nodesTested,
            nodesFound: nodesFound
          });
          ...
          console.danger('Whoops! We hit an error:', e);
          console.danger('Here\'s what we\'d found up until that error:');
          setDoneResults({
            nodesTested: nodesTested,
            nodesFound: nodesFound
          });
